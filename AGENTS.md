...
| Verify DB-free software | `pnpm test:software` |
| Verify all guardrails | `TEST_DATABASE_URL=... pnpm verify` |
...
11. Run `TEST_DATABASE_URL=... pnpm verify` with an explicit disposable database

### Risk-Weighted Verification

Tests exist to protect risk-bearing behavior, not to maximize test count. Use one global
OpenAPI contract suite for route registration and one real-app lifecycle per generated
CRUD slice. Add focused tests only for meaningful business rules, security boundaries,
failure recovery, or regressions.

Do not add per-slice contract duplicates, empty smoke tests, `it.todo()`, `.skip`, or
copy-pasted CRUD matrices. `pnpm test:software` is the fast DB-free lane used by default
on pre-push; `RUN_PREPUSH_TESTS=1 TEST_DATABASE_URL=... git push` opts into the full
local suite with an explicit disposable database. CI always runs both the software lane
and the full DB-backed suite.
...
- **Testing:** Admit tests by risk. Preserve global/static verifiers and one real lifecycle per exposed CRUD surface; add more only when they protect distinct behavior.
